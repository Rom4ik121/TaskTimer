...
Кратко по волнам **A–AU** (схема SQLite → `SCHEMA_VERSION = 13`). Wave **AU** — фильтры списка задач в отдельном листе.

Исторические волны Z–AL: SCHEMA_VERSION = 11 (без бампа до AM).

## AU — фильтры задач в отдельном окне

- Экран Задачи: убраны ряды чипов статуса / приоритета / сортировки / тегов с основного UI
- Одна кнопка «Фильтры» (воронка) в шапке + компактная сводка «Фильтр: В работе · Высокий», когда что-то выбрано
- `show_filter_sheet` в `dialogs.py`: BottomSheet со секциями Статус / Приоритет / Сортировка / Теги и действиями «Сбросить» + «Готово»
- Готово закрывает лист и обновляет список; свайп отменяет черновик
- Дом / Статы / Поиск / Холст: чипы дайджеста и недавних поисков — не фильтры списка (Wave AT nav), оставлены
